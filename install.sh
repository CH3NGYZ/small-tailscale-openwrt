...
# 预先计算的校验和
EXPECTED_CHECKSUM_SHA256="296fcace6d4613201863b6f17015612071c64e58aade613645a31a22cb4a3ecd"
EXPECTED_CHECKSUM_MD5="68bd0c5ec377d0a9e104994108667553"
TIME_OUT=30
...
if [ "$need_install" -eq 0 ]; then
    ...
    # 最终检查 timeout 和 nohup
    for cmd in timeout nohup; do
        if ! command -v $cmd >/dev/null 2>&1; then
            log_error "❌ 未检测到 $cmd 命令，尽管已尝试安装，脚本退出。"
            exit 1
        else
            log_info "✅ $cmd 命令已可用"
        fi
